...
func layer_getHash(path string, keypath string) string{
...
func create_signfile(path string, Signdata map[string]string){
    ...
    encodeFile.Close()
    //log.Printf("Signed signature data is in %s\n",path)
}
...
func image_layer_load(imageName string) bool {
		...
		for i := 0; i<len(layerIds); i++{
                	encodesign = append(encodesign, layer_getHash(layersPath[i],"./dev1/dev1.key"))
        	}
